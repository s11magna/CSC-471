...
public:
	...
	void initTex(const std::string& resourceDirectory)
	{
		...
		texture3->setFilename(resourceDirectory + "/world_maps_2.jpg");
		texture3->init();
		texture3->setUnit(3);
	...
	void initGeom(const std::string& resourceDirectory)
	{
		...
		string errStr;
		//load in the mesh and make the shapes for the figure
		...
		else
		{
			...
			for (size_t i = 0; i < TOshapes.size(); i++)
			{
				...
				// keep track of the Max and Min Globally
				Gmin = vec3(min(Gmin.x,temp->min.x), min(Gmin.y, temp->min.y), min(Gmin.z, temp->min.z));
				Gmax = vec3(max(Gmax.x, temp->max.x), max(Gmax.y, temp->max.y), max(Gmax.z, temp->max.z));
			...
			tempTrans = Gmin + 0.5f*( Gmax- Gmin);

			if (Gmax.x >Gmax.y && Gmax.x > Gmax.z)
			{
				dScale = 2.f / (Gmax.x - Gmin.x);
			}
			else if (Gmax.y > Gmax.x && Gmax.y > Gmax.z)
			{
				dScale = 2.f / (Gmax.y - Gmin.y);
			}
			else
			{
				dScale = 2.f / (Gmax.z - Gmin.z);
			}
		}
	...
	void initQuad()
	{
		...
		float GrndTex[] = { // ground texture coordinates
			0, 0, // back
			0, g_groundSize,
	...
	void render()
	{
		...
		for (size_t i = 0; i < AllShapes.size(); i++)
		{
			M->pushMatrix();
			{
				...
				M->translate(vec3(0, 0.f, -5));
				M->scale(dScale);
				M->rotate(radians(-90.f), vec3(1, 0, 0));
				M->rotate(radians(-90.f), vec3(0, 0, 1));
				M->translate(-1.0f * tempTrans);
				SetMaterial(1);
				glUniformMatrix4fv(prog->getUniform("M"), 1, GL_FALSE, value_ptr(M->topMatrix()));
		...
		M->pushMatrix();
			...
			/* World Texture*/
			M->pushMatrix();
			M->translate(vec3(-4, 0.f, -4));
			M->scale(gDScale);
			M->rotate(radians(-180.f), vec3(0, 1, 0));
			M->translate(-1.0f*gDTrans);
			glUniformMatrix4fv(prog->getUniform("M"), 1, GL_FALSE, value_ptr(M->topMatrix()));
